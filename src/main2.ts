...
class Initializer {
    ...
    constructor() {
        ...
        (window as any).removeFromCart = (id: string) => this.removeFromCart(id);
        (window as any).updateQuantity = (id: string, value: number) => this.updateQuantity(id, value);
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (event) => {
            if (this.element.nav?.classList.contains('active') && event.target !== this.element.nav && !this.element.nav.contains(event.target as Node)) {
    ...
    toggleNav() {
        ...
        // Add extra top space if promo is showing and no scroll yet 
    ...
    indicateProductInCart() {
        ...
        let products = document.querySelectorAll<HTMLElement>('.product-container')
    ...
    updateQuantity(itemId: string, value: number) {
        // Map to new CartItem objects and remove any items with quantity <= 0
